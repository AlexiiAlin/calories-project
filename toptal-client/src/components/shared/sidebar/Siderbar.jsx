...
import {componentStyles} from "./sidebar";
...
export default function Sidebar({ routes, logo, dropdown, input }) {
    ...
    const createLinks = (routes) => {
      return routes
        .filter(
          e => e.roles && e.roles.length !== 0
            ? userContext.user && userContext.user.userType && e.roles.indexOf(userContext.user.userType) !== -1 && !e.hideFromSidebar
            : !e.hideFromSidebar
        )
        .map((prop, key) => {
          if (prop.divider) {
    ...
    let logoImage = (
      <img alt={logo.imgAlt} className={classes.logoClasses + ' m-auto'} src={logo.imgSrc} />
    );
    let logoObject =
      logo && logo.innerLink ? (

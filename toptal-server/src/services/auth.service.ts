...
class AuthService {
  ...
  public async me(userEmail: string): Promise<{ user: User }> {
    if (isEmpty(userEmail)) throw new HttpException(400, "You're not userData");

    const userRepository = getRepository(this.users);
    const user: User = await userRepository.findOne({ where: { email: userEmail } });
    if (!user) throw new HttpException(409, `You're email ${userEmail} not found`);
  ...
  public createCookie(tokenData: TokenData): string {
    return `Authorization=${tokenData.token}; HttpOnly; Max-Age=${tokenData.expiresIn}; SameSite=None; Secure=true;`;
  }
}
